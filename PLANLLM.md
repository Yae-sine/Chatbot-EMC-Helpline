...
before implementation). Note: the legacy `text-embedding-001`/`text-embedding-004`
embedding family is **retired** (404 on v1beta since early 2026); the current
GA embedding model is `gemini-embedding-001` (supported until May 2028).
...
**Embeddings:** `gemini-embedding-001` (Gemini — GA since Jul 2025, up to 3,072
dims, MRL; French-capable; same provider as generation; shutdown May 2028 —
the legacy `text-embedding-001`/`text-embedding-004` family is retired,
404 on v1beta). Paid $0.15/M if exceeded.
...
`gemini-embedding-001` embeddings), **Groq** (Llama 3.3 70B or Qwen3 32B) as
...
- **External services:** **Gemini 2.5 Flash + gemini-embedding-001 (free tier)**
