...
data/
  qa-database.ts          # 75 QAEntry (typed), from docs/qa-source.md + Ressources PDF
...
- **`QA_DATABASE` (`data/qa-database.ts`):** 75 entries with unique `id`s in
...
- `data/qa-database.ts` — the 75-entry typed knowledge base used at runtime.
